...
const STACK = [
  {
    tag: 'Discovery',
    k: 'Reels & Stories',
    d: 'Vertical-native creative shot for the feed - not repurposed.',
  },
  {
    tag: 'Consider',
    k: 'Lead Forms',
    d: 'Pre-qualifying questions, instant CRM handoff, no manual export.',
  },
  {
    tag: 'Convert',
    k: 'Conversions API',
    d: 'Server-side events keep tracking accurate after iOS signal loss.',
  },
  {
    tag: 'Retain',
    k: 'WhatsApp Click-to-Chat',
    d: 'Recovery flow for the 70%+ of leads who never finish a form.',
  },
];
...
const TACTICS = [
  {
    n: '01',
    h: 'Audience layering',
    d: 'Layered geo + interest + custom audience stacks built per service line.',
  },
  {
    n: '02',
    h: 'Vertical creative kits',
    d: 'Editorial-grade creative scripts and edits, recorded in-clinic or remote.',
  },
  {
    n: '03',
    h: 'Pre-qualifying lead forms',
    d: 'Three-question filter that reduces tire-kickers without killing volume.',
  },
  {
    n: '04',
    h: 'Booking pixel + CAPI',
    d: 'Server-side conversions with deduped event IDs - what iOS hid, we surface.',
  },
  {
    n: '05',
    h: 'Compliant before/after',
    d: 'Aesthetic creative frameworks that survive Meta review and brand guidelines.',
  },
  {
    n: '06',
    h: 'Retention nudges',
    d: 'Win-back, recall, and reactivation audiences. Lookalikes from booked patients.',
  },
];
...
const RESULTS = [
  ...
  { k: 'ROAS', v: '4.6×', desc: 'Tracked return on Meta spend in aesthetic and dental verticals.' },
  {
    k: 'Quality',
    v: '+62%',
    desc: 'Improvement in lead-to-booked rate after pre-qualifying filters.',
  },
];
      ...
      <div className="mt-6 grid lg:grid-cols-12 gap-x-12 gap-y-10 items-end">
        <div className="lg:col-span-8">
          ...
          <p className="mt-7 text-body text-[17px] leading-[1.65] max-w-[58ch]">
            A Meta Ads programme that respects healthcare compliance, survives iOS signal loss, and
            books real patients. Not impressions. Not reach. Not lead-form spam. Bookings,
...
const Stack = () => (
  <Section
    no="01"
    title="The funnel, end to end"
    kicker="A four-layer stack across the Meta family. Each layer has a single, measurable job."
  >
      ...
      {STACK.map((s, i) => (
        <div key={s.k} className="bg-bg p-7 sm:p-8 flex flex-col gap-5 relative">
          <div className="flex items-center justify-between">
            <span className="font-mono text-[11px] text-cta tracking-[0.18em] font-semibold">
              0{i + 1}.
            </span>
          ...
          <h3 className="text-heading text-[24px] font-extrabold leading-[1.1] tracking-[-0.02em]">
            {s.k}
          </h3>
...
const Tactics = () => (
  <Section
    no="02"
    title="Tactics on the bench"
    kicker="Six moves we ship from week one. Layered by service line, not by template."
  >
    <ol className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-12">
      {TACTICS.map((t) => (
        <li key={t.n} className="border-t-2 border-heading pt-5">
          ...
          <h4 className="mt-4 text-heading text-[20px] font-bold tracking-[-0.015em] leading-[1.2]">
            {t.h}
          </h4>
...
const Results = () => (
  <Section
    ...
    kicker="A snapshot from the live aggregate. Pulled monthly from connected ad accounts."
  >
    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-[1px] bg-heading">
      {RESULTS.map((r, i) => (
        <div key={r.k} className="bg-bg p-7 flex flex-col gap-3">
          ...
          <div className="text-heading font-extrabold text-[56px] leading-[0.95] tracking-[-0.035em] tabular-nums">
            {r.v}
          </div>
    ...
    <p className="mt-6 text-muted text-[13px] leading-[1.6] max-w-[64ch]">
      ...
      audience frameworks are built against the current policy. We do not promise prohibited
      targeting.
    </p>
  </Section>
);
...
const Closing = () => (
  <Section
    no="04"
    title="Audit your Meta account"
    kicker="A two-week fixed-fee audit with a no-pitch readout. Whatever the verdict, you keep the audit."
  >
    <EditorialCTA
