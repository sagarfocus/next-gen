    ...
    <section className="sl-section an-how-section">
      <div className="container-shell">
        ...
        <p className="an-how-intro">
          A typical healthcare patient touches your brand 7-12 times before
          booking: Google search, AI Overview, GBP listing, Instagram reel,
          Facebook ad, email, friend referral, direct visit. Default GA4
          gives 100% of that journey to the <strong>last touch</strong> -
          usually brand search. The result: paid steals credit from SEO,
          SEO steals credit from social, and budget decisions get made on
          half a story. We run all four attribution models in parallel so
          you can argue from data instead of intuition.
        </p>
        ...
        <div className="an-how-grid">
          {MODELS.map((m, i) => (
            <article key={m.name} className="an-how-card" data-pos={i}>
              <div className="an-how-letter" aria-hidden="true">{m.letter}</div>
              ...
              <ul className="an-how-examples">
                {m.examples.map((e) => (
                  <li key={e}>
                    <span className="an-how-bullet" aria-hidden="true">→</span>
                    {e}
                  </li>
                ))}
              </ul>
        ...
        <div className="an-how-callout">
          ...
          <p>
            Healthcare patient journeys are long, multi-touch, and
            cross-device. A patient who Googles &ldquo;urgent care near
            me,&rdquo; sees your TikTok the next day, reads a friend&rsquo;s
            Google review, and then books via direct URL - that&rsquo;s a 4-
            touch journey GA4 records as <em>100% direct</em>. We deploy
            server-side conversion APIs, hashed-ID matching across devices,
            and the position-based model as the operational default. Budget
            decisions get made on the journey - not on the last click.
          </p>
        </div>
      </div>
    </section>
