...
async function handlePost(req: import('express').Request, res: import('express').Response) {
  try {
    const { messages, sessionId, visitorId, language } = (req.body) as ChatRequestBody;
    ...
    return res.json({ reply, sources, fallback: usedFallback, unanswered: isUnanswered });
  } catch (error) {
    console.error('Chat API error:', error);
    return res.json({ reply: "Oops, I hit a small snag! Please try again, or reach out to us at https://thenextgenhealth.com/contact. I'm Nex, and I'm here whenever you're ready! 😊", sources: [], fallback: true, unanswered: false });
  }
}
...
const router = Router();
router.post('/', (req, res) => {
  Promise.resolve(handlePost(req, res)).catch((err) => {
